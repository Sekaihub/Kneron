...
#define FAT_DPRINT(...)    VERBOSE("[FAT]" __VA_ARGS__)
#else
...
typedef struct boot_sector {
	...
	__u16	flags;		/* 0x028 Bit 8: fat mirroring, low 4: active fat */
	__u8	version[2];	/* 0x02A Filesystem version */
	__u32	root_cluster;	/* 0x02C First cluster in root directory */
	__u16	info_sector;	/* 0x030 Filesystem info sector */
	__u16	backup_boot;	/* 0x032 Backup boot sector */
	__u16	reserved2[6];	/* 0x034 Unused */
} boot_sector;

typedef struct volume_info
{
	__u8 drive_number;	/* 0x040 BIOS drive number */
	__u8 reserved;		/* 0x041 Unused */
	__u8 ext_boot_sign;	/* 0x042 0x29 if fields below exist (DOS 3.3+) */
	__u8 volume_id[4];	/* 0x043 Volume ID number */
	char volume_label[11];	/* 0x047 Volume label */
	char fs_type[8];	/* 0x052 Typically FAT12, FAT16, or FAT32 */
	char pad[1];
...
typedef struct {
	...
	__u32	clust_size;	/* Size of clusters in sectors */
...
void disk_set_dev(block_dev_desc_t * dev);
#define fatroundup(x, y)           ((((x) + ((y) - 1)) / (y)) * (y))
