...
#include <drivers/vatics/mmc/mmc.h>
#include <export/common/bl_common_exp.h>
#include <linux/dma-direction.h>

#undef CONFIG_DMA_ADDR_T_64BIT
...
#define INSPEL_CNFG_BITS	(U(1)<<2 | U(1)<<3)
#define INSPEL_CNFG_OFFSET	0x2

#define PHY_RSTN		(1U<<0)
#define PHY_PWRGOOD		(1U<<1)

#define SDHCI_VENDOR_SPECIFIC   0x500
#define SDHCI_EMMC_CTRL_R       SDHCI_VENDOR_SPECIFIC + 0x2C
#define SDHCI_BOOT_CTRL_R       SDHCI_VENDOR_SPECIFIC + 0x2E
...
static inline void sdhci_writel(struct sdhci_host *host, u32 val, int reg)
{
	writel(val, (unsigned long)host->ioaddr + reg);
}

static inline void sdhci_writew(struct sdhci_host *host, u16 val, int reg)
{
	writew(val, (unsigned long)host->ioaddr + reg);
}

static inline void sdhci_writeb(struct sdhci_host *host, u8 val, int reg)
{
	writeb(val, (u64)host->ioaddr + reg);
}
static inline u32 sdhci_readl(struct sdhci_host *host, int reg)
{
	return readl((unsigned long)host->ioaddr + reg);
}

static inline u16 sdhci_readw(struct sdhci_host *host, int reg)
{
	return readw((unsigned long)host->ioaddr + reg);
}

static inline u8 sdhci_readb(struct sdhci_host *host, int reg)
{
	return readb((unsigned long)host->ioaddr + reg);
}
#endif
...
#define SDHCI_TRANSFER_SIZE_BL2 18 * 1024
#define SDHCI_TRANSFER_SIZE_BL3  8 * 1024 * 1024

#ifdef IMAGE_BL1
/* this is used for BL1 to transfer BL2 */
#define SDHCI_TRANSFER_SIZE	BL2_PACKSIZE_DEFAULT
#elif IMAGE_BL2
#define SDHCI_TRANSFER_SIZE	SDHCI_TRANSFER_SIZE_BL2
#else
#define SDHCI_TRANSFER_SIZE	SDHCI_TRANSFER_SIZE_BL3
#endif /* end if of IMAGE_BL1 */
