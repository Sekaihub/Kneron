...
#if 1
#define usberr(fmt, args...) ERROR("ERROR: %s(), %d: "fmt"\n", \
...
#if 0
#define usbdbg(fmt, args...) VERBOSE("debug: %s(), %d: "fmt"\n", \
				 __function__, __LINE__, ##args)
#else
...
#if 0
#define usbinfo(fmt, args...) INFO("info: %s(), %d: "fmt"\n", \
				  __function__, __LINE__, ##args)
#else
 ...
 *
 */
typedef enum usb_device_state_2 {
...
{
	INFO("usb request\n");
	INFO("\tbmRequestType 0x%2.2x\n", r->bmRequestType);
	if ((r->bmRequestType & USB_REQ_DIRECTION_MASK) == 0)
		INFO("\t\tDirection : To device\n");
	else
		INFO("\t\tDirection : To host\n");
	...
	if ((r->bmRequestType & USB_REQ_RECIPIENT_MASK) ==
		...
		INFO("\t\tRecipient : Device\n");
	if ((r->bmRequestType & USB_REQ_RECIPIENT_MASK) ==
	    USB_REQ_RECIPIENT_INTERFACE)
		INFO("\t\tRecipient : Interface\n");
	if ((r->bmRequestType & USB_REQ_RECIPIENT_MASK) ==
	    USB_REQ_RECIPIENT_ENDPOINT)
		INFO("\t\tRecipient : Endpoint\n");
	if ((r->bmRequestType & USB_REQ_RECIPIENT_MASK) ==
	    USB_REQ_RECIPIENT_OTHER)
		INFO("\t\tRecipient : Other\n");
	INFO("\tbRequest      0x%2.2x\n", r->bRequest);
	...
	if (r->bRequest == USB_REQ_GET_DESCRIPTOR) {
		...
		case USB_DESCRIPTOR_TYPE_DEVICE_QUALIFIER:
			INFO("\tDEVICE_QUALIFIER\n");
			break;
		case USB_DESCRIPTOR_TYPE_OTHER_SPEED_CONFIGURATION:
			INFO("\tOTHER_SPEED_CONFIGURATION\n");
			break;
		case USB_DESCRIPTOR_TYPE_INTERFACE_POWER:
			INFO("\tINTERFACE_POWER\n");
			break;
