...
#include <drivers/vatics/usb/gadget/u_os_desc.h>
#include <assert.h>
#include <wagner_buf.h>
#include <wagner_log.h>

#define debug DBGUSB
#define USB_GADGET_OS_DESCRIPTORS 1
...
static const char qw_sign_buf[OS_STRING_QW_SIGN_LEN / 2] = {
			'M', 'S', 'F', 'T', '1', '0', '0'};
		...
		struct usb_function *function)
{
	...
	assert(function->set_alt && function->disable);
...
/*
	...
	assert(id < MAX_CONFIG_INTERFACES);
	...
	return -ENODEV;
*/
	config->interface[0] = function;
	config->next_interface_id = 1;
	return 0;
}
...
static void device_qual(struct usb_composite_dev *cdev)
{
	...
	// = count_configs(cdev, USB_DT_DEVICE_QUALIFIER);
	qual->bRESERVED = 0;
}
...
{
	/***********************************/
	// Do not reset in TF-A
	// Keep cdev->config = g_dnl_config
	/***********************************/
#if 0
	struct usb_function		*f;
		...
		struct usb_configuration *config)
{
	...
	assert((config->bConfigurationValue) && (config->bind));

	cdev->config = config; // TF-A always use 1 config and store here
	config->cdev = cdev;
...
//	list_for_each_entry(c, &cdev->configs, list) {
	if (1) {
		...
		if (f) {
			if (f->strings) {
				len = lookup_string(f->strings, buf,
						language, id);
				if (len > 0)
					return len;
			}
...
#if (USB_GADGET_OS_DESCRIPTORS == 1)
...
static int composite_bind(struct usb_gadget *gadget)
{
...
#ifdef IMAGE_BL2
	cdev = &overlaid_bufferp->usb_buf.g_cdev;
#else
	cdev = &overlaid_buffer.usb_buf.g_cdev;
#endif
	...
	cdev->req = usb_ep_alloc_request(gadget->ep0, 0);
	if (!cdev->req)
		goto fail;
...
#ifdef IMAGE_BL2
	cdev->req->buf = overlaid_bufferp->usb_buf.composite_req_buf;
#else
	cdev->req->buf = overlaid_buffer.usb_buf.composite_req_buf;
#endif
	memset(cdev->req->buf, 0, USB_BUFSIZ);
...
composite_suspend(struct usb_gadget *gadget)
{
	...
	debug("%s: suspend\n", __func__);
	wagner_bootstep(WAGNER_STEP_USB_SUSPEND);
	if (cdev->config) {
		//list_for_each_entry(f, &cdev->config->functions, list) {
		f = cdev->config->interface[0];
		if (f) {
...
int usb_composite_register(struct usb_composite_driver *driver)
{
	...
	assert(driver && driver->dev && driver->bind && !composite);
	assert(driver->name);
