...
static struct usb_endpoint_descriptor fs_ep_in = {
	...
	.bEndpointAddress   = USB_DIR_IN | FB_EPNUM,
...
static struct usb_endpoint_descriptor fs_ep_out = {
	.bLength		= USB_DT_ENDPOINT_SIZE,
	.bDescriptorType	= USB_DT_ENDPOINT,
	.bEndpointAddress	= USB_DIR_OUT | FB_EPNUM,
	.bmAttributes		= USB_ENDPOINT_XFER_BULK,
	.wMaxPacketSize		= cpu_to_le16(64),
};
	    ...
	    struct usb_endpoint_descriptor *ss)
{
	if (gadget_is_superspeed(g) && g->speed >= USB_SPEED_SUPER) {
		DBGUSB("fb_ep_desc=ss\n");
		return ss;
	}

	if (gadget_is_dualspeed(g) && g->speed == USB_SPEED_HIGH) {
		DBGUSB("fb_ep_desc=hs\n");
		return hs;
	}
	DBGUSB("fb_ep_desc=fs\n");
	return fs;
}
...
#ifdef IMAGE_BL2
	dep = &overlaid_bufferp->usb_buf.dep[(FB_EPNUM << 1) | (!!USB_DIR_IN)];
#else
	dep = &overlaid_buffer.usb_buf.dep[(FB_EPNUM << 1) | (!!USB_DIR_IN)];
#endif
	f_fb->in_ep = &dep->endpoint;
	if (!f_fb->in_ep)
		return -ENODEV;
	f_fb->in_ep->driver_data = c->cdev;

#ifdef IMAGE_BL2
	dep = &overlaid_bufferp->usb_buf.dep[(FB_EPNUM << 1) | (!!USB_DIR_OUT)];
#else
	dep = &overlaid_buffer.usb_buf.dep[(FB_EPNUM << 1) | (!!USB_DIR_OUT)];
#endif
	f_fb->out_ep = &dep->endpoint;
	if (!f_fb->out_ep)
		return -ENODEV;
...
static struct usb_request *fastboot_start_ep(struct usb_ep *ep, int dir)
{
...
#ifdef IMAGE_BL2
	if (dir == USB_DIR_OUT)
		req->buf = overlaid_bufferp->usb_buf.fb_req_buf_out;
	else
		req->buf = overlaid_bufferp->usb_buf.fb_req_buf_in;
#else
	if (dir == USB_DIR_OUT)
		req->buf = overlaid_buffer.usb_buf.fb_req_buf_out;
	else
		req->buf = overlaid_buffer.usb_buf.fb_req_buf_in;
			    ...
			    unsigned interface, unsigned alt)
{
	...
	if (ret) {
		ERROR("failed to enable out ep\n");
		return ret;
	}

	f_fb->out_req = fastboot_start_ep(f_fb->out_ep, USB_DIR_OUT);
	if (!f_fb->out_req) {
		ERROR("failed to alloc out req\n");
		ret = -EINVAL;
		goto err;
	}
	...
	if (ret) {
		ERROR("failed to enable in ep\n");
		goto err;
	}

	f_fb->in_req = fastboot_start_ep(f_fb->in_ep, USB_DIR_IN);
	if (!f_fb->in_req) {
		ERROR("failed alloc req in\n");
		ret = -EINVAL;
		goto err;
	}
...
#ifdef IMAGE_BL2
	fastboot_func = &overlaid_bufferp->usb_buf.f_fb;
#else
	fastboot_func = &overlaid_buffer.usb_buf.f_fb;
#endif
	memset(fastboot_func, 0, sizeof(*fastboot_func));

	fastboot_func->usb_function.name = "f_fastboot";
	fastboot_func->usb_function.bind = fastboot_bind;
	fastboot_func->usb_function.unbind = fastboot_unbind;
	fastboot_func->usb_function.set_alt = fastboot_set_alt;
	fastboot_func->usb_function.disable = fastboot_disable;
	fastboot_func->usb_function.strings = fastboot_strings;

	status = usb_add_function(c, &fastboot_func->usb_function);
...
static void rx_handler_dl_image(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (response[0]) {
		...
		image_size = fastboot_data_complete(response);
		g_dnl_set_imagesize(image_size);
...
{
	// Back to BL1 authentication, do not boot here.
	// Remember to skip 2K Android header.
}
...
static void rx_handler_command(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (req->status != 0 || req->length == 0) {
		WARN("req status=%d, req length=%d\n",
	...
	if (req->actual < req->length) {
		...
		ERROR("buffer overflow, actual=%d, len=%d\n",
