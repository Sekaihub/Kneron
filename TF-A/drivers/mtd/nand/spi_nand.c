...
#define SPINAND_ID(...)	(const uint8_t[]){ __VA_ARGS__ }

#pragma weak plat_get_spi_nand_data
int plat_get_spi_nand_data(struct spinand_device *device)
{
	device->nand_dev->block_size = SZ_2K * 64;
	device->nand_dev->page_size = SZ_2K;
	device->nand_dev->size = SZ_2K * 64 * 10;
	return 0;
}
...
static int spi_nand_cont_enable_mxic(void)
{
	return spi_nand_update_cfg(BIT(2), BIT(2));
}

static int spi_nand_cont_disable_mxic(void)
{
	return spi_nand_update_cfg(BIT(2), 0U);
}

static int spi_nand_cont_postxfer_mxic(void)
{
	udelay(6);
	return 0;
}

static int spi_nand_cont_enable_w25n01gv(void)
{
	return spi_nand_update_cfg(BIT(3), 0U);
}

static int spi_nand_cont_disable_w25n01gv(void)
{
	return spi_nand_update_cfg(BIT(3), BIT(3));
}
...
static int spi_nand_read_cont(struct nand_device *nand, unsigned int page,
			      unsigned int offset, uint8_t *buffer,
			      unsigned int len, bool ecc_enabled)
{
	uint8_t status;
	int ret = 0;
	...
	if (nand->cont_info->cont_enable) {
		ret = nand->cont_info->cont_enable();
		if (ret != 0) {
			return ret;
		}
	}
	...
	if (nand->cont_info->cont_pre_xfer) {
		ret = nand->cont_info->cont_pre_xfer();
		if (ret != 0) {
			return ret;
		}
	}
	...
	if (nand->cont_info->cont_post_xfer) {
		ret = nand->cont_info->cont_post_xfer();
		if (ret != 0) {
			return ret;
		}
	}
	...
	if (nand->cont_info->cont_disable) {
		ret = nand->cont_info->cont_disable();
		if (ret != 0) {
			return ret;
		}
	}
...
static int spi_nand_mtd_read(struct nand_device *nand, unsigned int page,
		unsigned int offset, uintptr_t buffer, unsigned int len)
{
	return spi_nand_read_page(page, offset, (uint8_t *)buffer,
				  len, true);
}

static int spi_nand_mtd_read_cont(struct nand_device *nand, unsigned int page,
		unsigned int offset, uintptr_t buffer, unsigned int len)
{
	return spi_nand_read_cont(nand, page, offset, (uint8_t *)buffer,
				  len, true);
}
...
static const struct cont_info cont_allow_list[] = {
	{
		.model = "MX35LF4GE4AD",
		.id = SPINAND_ID(0xC2, 0x37),
		.id_len = 2,
		.enable = true,
		.cont_enable = spi_nand_cont_enable_mxic,
		.cont_disable = spi_nand_cont_disable_mxic,
		.cont_pre_xfer = NULL,
		.cont_post_xfer = spi_nand_cont_postxfer_mxic,
	},
	{
		.model = "W25N01GV",
		.id = SPINAND_ID(0xEF, 0xAA, 0x21),
		.id_len = 3,
		.enable = true,
		.cont_enable = spi_nand_cont_enable_w25n01gv,
		.cont_disable = spi_nand_cont_disable_w25n01gv,
		.cont_pre_xfer = NULL,
		.cont_post_xfer = NULL,
	}
};

static int spi_nand_cont_scan(uint8_t *id)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(cont_allow_list); i++) {
		if (!cont_allow_list[i].enable)
			continue;

		if (memcmp(id + 1, cont_allow_list[i].id,
			cont_allow_list[i].id_len))
			continue;

		spinand_dev.nand_dev->cont_info = &cont_allow_list[i];
		VERBOSE("continuous read %s\n", cont_allow_list[i].model);
		return 1;
	}
	spinand_dev.nand_dev->cont_info = NULL;
		  ...
		  unsigned int buswidth)
{
	...
	spinand_dev.nand_dev->mtd_read = spi_nand_mtd_read;
	...
	if (spinand_dev.cont_en) {
		ret = spi_nand_cont_scan(id);
		if (ret) {
			spinand_dev.nand_dev->mtd_read = spi_nand_mtd_read_cont;
		}
	}
	...
	if (buswidth == 4) {
		/* quad mode read command used (1-1-4 mode) */
		op = &spinand_dev.spi_read_cache_op;
		op->cmd.opcode = SPI_NAND_OP_READ_FROM_CACHE_4X;
		op->cmd.buswidth = SPI_MEM_BUSWIDTH_1_LINE;
		op->addr.nbytes = 2U;
		op->addr.buswidth = SPI_MEM_BUSWIDTH_1_LINE;
		op->dummy.nbytes = 1U;
...
struct spinand_device *get_spinand_device(void)
{
	return &spinand_dev;
}
