...
static inline int spi_nor_exit_4b(void)
{
	return spi_nor_reg(SPINOR_OP_EX4B, NULL, 0U, SPI_MEM_DATA_OUT);
}
		 ...
		 unsigned int buswidth)
{
	...
	*size = nor_dev.size;
	...
	VERBOSE("flash id = 0x%x 0x%x 0x%x\n", id[0], id[1], id[2]);
	...
	if (buswidth == 4) {
		/* quad mode read command used (1-4-4 mode) */
		nor_dev.read_op.cmd.opcode = SPI_NOR_OP_READ_1_4_4;
		nor_dev.read_op.cmd.buswidth = SPI_MEM_BUSWIDTH_1_LINE;
		nor_dev.read_op.addr.nbytes = 3U;
		nor_dev.read_op.addr.buswidth = SPI_MEM_BUSWIDTH_4_LINE;
		nor_dev.read_op.dummy.nbytes = 3U;
		nor_dev.read_op.dummy.buswidth = SPI_MEM_BUSWIDTH_4_LINE;
		nor_dev.read_op.data.buswidth = SPI_MEM_BUSWIDTH_4_LINE;
		nor_dev.read_op.data.dir = SPI_MEM_DATA_IN;
	}
