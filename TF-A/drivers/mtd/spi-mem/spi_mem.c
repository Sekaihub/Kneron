...
int spi_mem_init_slave(void *fdt, int bus_node, const struct spi_bus_ops *ops)
{
	...
	struct dw_qspi_config *qspi_config = (struct dw_qspi_config *)fdt;
...
#ifdef PLAT_wagner
	spi_slave.cs = qspi_config->cs;
	spi_slave.max_hz = qspi_config->max_hz;
	...
	if (qspi_config->cs_high) {
		mode |= SPI_CS_HIGH;
	}
	if (qspi_config->three_wire) {
		mode |= SPI_3WIRE;
	}
	if (qspi_config->half_duplex) {
		mode |= SPI_PREAMBLE;
	}

	switch (qspi_config->tx_bus_width) {
	case 1U:
		break;
	case 2U:
		mode |= SPI_TX_DUAL;
	...
	default:
		WARN("spi-tx-bus-width %u not supported\n",
			qspi_config->tx_bus_width);
		return -EINVAL;
	}

	switch (qspi_config->rx_bus_width) {
	case 1U:
		break;
	case 2U:
		mode |= SPI_RX_DUAL;
