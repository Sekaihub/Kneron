...
static void sdhci_adma_desc(struct sdhci_adma_desc *desc,
			    dma_addr_t addr, u16 len, int end)
{
	u8 attr;
			      ...
			      struct mmc_data *data, dma_addr_t addr)
{
	...
	while (--i) {
		sdhci_adma_desc(desc, addr, ADMA_MAX_LEN, 0);
		addr += ADMA_MAX_LEN;
		trans_bytes -= ADMA_MAX_LEN;
		desc++;
	}

	sdhci_adma_desc(desc, addr, trans_bytes, 1);

	flush_dcache_range((uintptr_t)table,
 ...
 * error.
 */
unsigned long sdhci_dma_table[20];
struct sdhci_adma_desc *sdhci_adma_init(void)
{
	return (struct sdhci_adma_desc *)sdhci_dma_table;
}
