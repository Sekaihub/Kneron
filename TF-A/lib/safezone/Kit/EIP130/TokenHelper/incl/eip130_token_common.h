...
#define EIP130TOKEN_OPCODE_EMMC                   ((Eip130TokenWord_t)10UL << 24)
#define EIP130TOKEN_OPCODE_EXT_SERVICE            ((Eip130TokenWord_t)11UL << 24)
...
/* eMMC operations */
#define EIP130TOKEN_SUBCODE_EMMC_RDREQ            ((Eip130TokenWord_t)0UL << 28)
#define EIP130TOKEN_SUBCODE_EMMC_RDVER            ((Eip130TokenWord_t)1UL << 28)
#define EIP130TOKEN_SUBCODE_EMMC_RDWRCNTREQ       ((Eip130TokenWord_t)2UL << 28)
#define EIP130TOKEN_SUBCODE_EMMC_RDWRCNTVER       ((Eip130TokenWord_t)3UL << 28)
#define EIP130TOKEN_SUBCODE_EMMC_WRREQ            ((Eip130TokenWord_t)4UL << 28)
#define EIP130TOKEN_SUBCODE_EMMC_WRVER            ((Eip130TokenWord_t)5UL << 28)
/* Service operations */
...
#define EIP130TOKEN_SUBCODE_HIBERNATION           ((Eip130TokenWord_t)6UL << 28)
#define EIP130TOKEN_SUBCODE_RESUMEFROMHIBERNATION ((Eip130TokenWord_t)7UL << 28)
 ...
 *     <0   Error code
 *     >0   warning code
 *   FAsvc bit is returned on bit position 16
 */
        ...
        const Eip130Token_Result_t * const ResultToken_p)
{
    ...
    FAsvc = ResultToken_p->W[0] & BIT_16;

    if ((result & (Eip130TokenWord_t)BIT_7) != 0U)
    {
        rv = -(Eip130FunctionStatus_t)((result & MASK_7_BITS));
        if (FAsvc == 0)
        {
             rv = (Eip130FunctionStatus_t)((int)rv & (int)(~BIT_16));
        }
    }
    else
    {
        rv = (Eip130FunctionStatus_t)(result | FAsvc);
    }
    return rv;
}
