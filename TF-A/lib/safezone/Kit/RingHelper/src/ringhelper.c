    ...
    const int DescriptorCount)
{
    ...
    if (Ring_p->fSeparate)
    {
        ...
        else
        {
            ...
            if (Ring_p->IN_Tail < Device_IN_Head)
            {
                ...
                A = (int)(Device_IN_Head - Ring_p->IN_Tail);
            }
            else
            {
                // used positions are between the two pointers
                // rest is free
                A = (int)(Ring_p->IN_Size - (Ring_p->IN_Tail - Device_IN_Head));
            }
    ...
    else
    {
        ...
        if (Ring_p->IN_Tail < OUT_Head_copy)
        {
            ...
            A = (int)(OUT_Head_copy - Ring_p->IN_Tail);
        }
        else
        {
            // used positions are between the two pointers
            // rest is free
            A = (int)(Ring_p->IN_Size - (Ring_p->IN_Tail - OUT_Head_copy));
        }
    ...
    W1 = Ring_p->CB.WriteFunc_p(
             ...
             /*WriteCount:*/(unsigned int)N,
             /*AvailableSpace*/(unsigned int)A,
             Descriptors_p,
             DescriptorCount,
             /*SkipCount:*/0);
    if (W1 <= 0)
            ...
            A > N)
    {
        ...
        W2 = Ring_p->CB.WriteFunc_p(
                 ...
                 /*WriteIndex:*/0,
                 /*WriteCount:*/(unsigned int)N,
                 /*AvailableSpace*/(unsigned int)N,
                 Descriptors_p,
                 DescriptorCount,
                 /*SkipCount:*/(unsigned int)W1);
        if (W2 < 0)
        {
            // failure
            return W2;      // ## RETURN ##
    ...
    {
        unsigned int i = Ring_p->IN_Tail + (unsigned int)(W1 + W2);
    ...
    const int DescriptorsLimit)
{
    ...
    A = (int)(Ring_p->OUT_Size - Ring_p->OUT_Head);
    ...
    {
        unsigned int i = Ring_p->OUT_Head + (unsigned int)(R1 + R2);
