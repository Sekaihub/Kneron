    ...
    const bool fExternalTrigger)
{
    EIP93_Write32(Device,
                  ...
                  ((unsigned int)(RingSize - 1) & MASK_10_BITS) |
    ...
    bool * const fExternalTrigger)
{
    ...
    if (RingSize)
    {
         *RingSize = (uint16_t)((word & MASK_10_BITS) + 1);
    }
    if (fExternalTrigger)
    {
    ...
    bool * const fTimeoutEnable_p)
{
    ...
    if (ResRingTimeOut_p)
    {
        *ResRingTimeOut_p = (uint16_t)((word >> 26) & MASK_6_BITS);
    }
    if (fTimeoutEnable_p)
    {
    ...
    uint16_t * const PeOutputSize)
{
    ...
    if (PeOutputSize)
    {
        *PeOutputSize = (uint16_t)((word >> 22) & MASK_10_BITS);
    }
}
    ...
    const unsigned int NumberOfDescriptors)
{
    ...
    const unsigned int nDescrSize = EIP93_ARM_DESCRIPTOR_SIZE();
    unsigned int i;
...
#endif /* INCLUDE_GUARD_EIP93_ARM_LEVEL0_H */
