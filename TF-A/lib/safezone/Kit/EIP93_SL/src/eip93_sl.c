...
{
    uint32_t word = 0;
    word |= (uint32_t)CommandCtrl_p->PadControl << 24;
    word |= (uint32_t)CommandCtrl_p->NextHeaderValue << 8;
    word |= (uint32_t)(CommandCtrl_p->fHashFinal ? 1 : 0) << 4;
    return word;
}
    ...
    EIP93_ResultDescriptor_Status_t * const  ResultStatus_p)
{
    ...
    ResultStatus_p->PadStatus = (uint8_t)((StatusWord >> 24) & (BIT_8 - 1));
