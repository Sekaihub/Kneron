    ...
    const unsigned DescriptorSkipCount)
{
    ...
    if (nWritten > 0)
    {
        DMAResource_PreDMA(ARM_p->CommandRingHandle,
                           ...
                           (unsigned int)nWritten * nDescrSize * 4);
    ...
    unsigned int * const DoneCount_p)
{
    ...
    EIP93_Read32_PE_CD_COUNT(Device_p->Device, &CDCount);
    EIP93_Read32_PE_RING_SIZE(Device_p->Device,
                              &RingSizeInDescriptor,
                              &ExternalTrigger);

    if(CDCount < (uint32_t)RingSizeInDescriptor)
    {
        *DoneCount_p = (unsigned int)RingHelper_Put(&ARM_p->RingHelper,
                                                    CmdDescriptors_p,
                                                    (int)CmdDescriptorCount);
    }
    else
    {
        *DoneCount_p = 0;
    ...
    unsigned int * const DoneCount_p)
{
    ...
    if(DoneCnt)
    {
        *DoneCount_p = (unsigned int)RingHelper_Get(&ARM_p->RingHelper,
                                                    -1, /* ReadyCount we don't know */
                                                    ResDescriptors_p,
                                                    (int)ResDescriptorLimit);
    }
    else
    {
        *DoneCount_p = 0;
