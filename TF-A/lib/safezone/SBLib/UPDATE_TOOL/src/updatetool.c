...
static bool
header_is_integrity_only(SBIF_Header_t * const Header_p)
{
    uint32_t image_type = BE32_READ(&Header_p->Type);
...
static
SB_Result_t sbverify(SBIF_Header_t * const Header_p,
                     ...
                     const void * const PublicKey_p)
{
    ...
    return SB_Image_Verify(&StorageArea,
                           NULL,
                           PublicKey_p,
                           NULL,
                           Header_p,
...
main(int argc, char * argv[])
{
    ...
    if (success && (sz > sizeof(SBIF_Header_t)))
    {
        SBIF_Header_t * const header_p = (void *)ptr;
        size_t hdrSize = SBIF_GET_HEADER_SIZE(header_p, sz);
        SBIF_SGVector_t vec;
        ...
        vec.DataLen = (uint32_t)(sz - hdrSize);
