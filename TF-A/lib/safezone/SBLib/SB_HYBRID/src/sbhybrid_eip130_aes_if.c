...
#define SBHYBRID_AESCBCKEY_POLICY   (SBHYBRID_ACCESS_POLICY           | \
                                     EIP130_ASSET_POLICY_SYM_BULK_AES | \
                                     EIP130_ASSET_POLICY_SCMCBCBC     | \
                                     EIP130_ASSET_POLICY_SCDIRDECVRFY)

#define SBHYBRID_AESWRAPKEY_POLICY  (SBHYBRID_ACCESS_POLICY           | \
                                     EIP130_ASSET_POLICY_SYM_WRAP     | \
                                     EIP130_ASSET_POLICY_SCAWKEY      | \
                                     EIP130_ASSET_POLICY_SCDIRDECVRFY)

#define SBHYBRID_KEYDERIVE_POLICY   (SBHYBRID_ACCESS_POLICY          | \
                                     EIP130_ASSET_POLICY_SYM_DERIVE  | \
                                     EIP130_ASSET_POLICY_SCADTRUSTED)
#else
#define SBHYBRID_AESCBCKEY_POLICY   (SBHYBRID_ACCESS_POLICY              | \
                                     EIP130_ASSET_POLICY_ALGO_CIPHER_AES | \
                                     EIP130_ASSET_POLICY_MODE2           | \
                                     EIP130_ASSET_POLICY_DECRYPT)

#define SBHYBRID_AESWRAPKEY_POLICY  (SBHYBRID_ACCESS_POLICY       | \
                                     EIP130_ASSET_POLICY_AES_WRAP | \
                                     EIP130_ASSET_POLICY_DECRYPT)

#define SBHYBRID_KEYDERIVE_POLICY   (SBHYBRID_ACCESS_POLICY                 | \
                                     EIP130_ASSET_POLICY_TRUSTED_KEY_DERIVE)
#endif
                     ...
                     const uint32_t DeriveInfoSize)
{
    ...
    {
#ifdef SBLIB_CF_IMAGE_TYPE_W_SBCR_KEY
#error "Not allow, because EIP-130 cannot load data into an asset that has the policy TRUSTED-KEY-DERIVE"
#else
        /* Search for asset (SBLIB_CFG_XM_IMAGE_TYPE_W_ASSET_KEY) and
           use it for AES Key Unwrap. */
        if (AES_IF_Ctx_LoadKeyData(Ctx_p,
                                   NULL,
                                   SBHYBRID_KEYDERIVE_POLICY,
                                   SBIF_CFG_CONFIDENTIALITY_BITS) == 0)
        {
