                            ...
                            const uint8_t InputBytes_p[16])
{
    ...
    // put the command in the ring (and start processing)
    {
        ...
        res = (int)EIP93_ARM_PacketPut(&Context_p->EIP93_IOArea,
                                       Cmds, CmdCount,
                                       &DoneCount);
                             ...
                             unsigned int InputByteCount)
{
    ...
    InputAddrPair.Address.Native_p = discard_const(InputBytes_p);
                            ...
                            uint8_t IVOutput_p[16])
{
    ...
    do
    {
        ...
        res = SBHYBRID_SHA2XX_RunFsm(Context_p);
...
{
    L_TRACE(LF_SBHYBRID, "Initializing EIP93 AES_IF implementation");
                                  ...
                                  const uint32_t KWKBits)
{
    ...
    for (j = 5; j >= 0; j--)
    {
        for (i = (int)n; i >= 1; i--)
        {
            u.B[2] = R[i - 1][0];
            u.B[3] = R[i - 1][1];
            cnt = (uint16_t)(n * (uint32_t)j + (uint32_t)i);
            u.A_bytes[7] ^= (uint8_t)cnt;
