         ...
         const int KWKBits)
{
    ...
    for(j = 0; j <= 5; j++)
    {
        for(i = 1; i <= n; i++)
        {
            ...
            cnt++;
            u.A_bytes[7] ^= (uint8_t)(cnt >> 0);
            u.A_bytes[6] ^= (uint8_t)(cnt >> 8);
            R[i - 1][0] = u.B[2];
            R[i - 1][1] = u.B[3];
        }
    }
               ...
               const int KWKBits)
{
    ...
    c_memcpy(R, ((char *)discard_const(WrappedIn_p)) + 8, n * 8);
                     ...
                     const uint32_t DeriveInfoSize)
{
    ...
    */
    /* Start inner hash */
    (void)SBHYBRID_SW_SHA256_Init(&SymmContext);
    ...
    /* Hash the padded key first */
    (void)SBHYBRID_SW_SHA256_AddBlock(&SymmContext, DeriveBuf, 64, false);

    /* Hash the 'info' string */
    (void)SBHYBRID_SW_SHA256_AddBlock(&SymmContext, DeriveInfo_p, DeriveInfoSize, false);

    /* Use Security Module (EIP-130) key derivation method
     * Format additional hash string
     */
    i = 0;
    DeriveBuf[i++] = 0;

    /* Host ID (1 byte) */
#ifdef SBIF_CFG_DERIVE_HOSTID
    DeriveBuf[i++] = (uint8_t)(1 << (SBIF_CFG_DERIVE_HOSTID - 1));
#else
    DeriveBuf[i++] = (uint8_t)1;
#endif

    /* Policy (8 bytes) for AES-Unwrap key (KEK) */
#if (SBIF_CFG_DERIVE_METHOD == 1)
    DeriveBuf[i++] = 0x00;
    DeriveBuf[i++] = 0x00;
    DeriveBuf[i++] = 0x00;
    DeriveBuf[i++] = 0x20;
    ...
    DeriveBuf[i++] = 0x00;
    DeriveBuf[i++] = 0x00;
#ifdef SBIF_CFG_DERIVE_HOSTSECURE
    DeriveBuf[i++] = 0x00;              /* Host connection is Secure */
#else
    DeriveBuf[i++] = 0x01;              /* Host connection is non-Secure */
#endif
#else /* (SBIF_CFG_DERIVE_METHOD == 2) == default one */
    DeriveBuf[i++] = 0x01;
#ifdef SBIF_CFG_DERIVE_HOSTSECURE
    DeriveBuf[i++] = 0x28;              /* Host connection is Secure */
#else
    DeriveBuf[i++] = 0x29;              /* Host connection is non-Secure */
#endif
    DeriveBuf[i++] = 0x25;
    DeriveBuf[i++] = 0x08;
    DeriveBuf[i++] = 0x00;
    ...
    /* FIPS state (1 byte) */
#ifdef SBIF_CFG_DERIVE_HOSTFIPS
    DeriveBuf[i++] = 0x56;              /* FIPS mode */
#else
    DeriveBuf[i++] = 0xA9;              /* Non-FIPS mode */
#endif

    /* Asset length (2 bytes, Key length in bytes) */
    DeriveBuf[i++] = (uint8_t)(SBIF_CFG_CONFIDENTIALITY_BITS / 8);
    DeriveBuf[i++] = 0;

    /* Iterator (1 byte) that starts at one */
    DeriveBuf[i++] = 1;

    /* Hash the additional hash string */
    (void)SBHYBRID_SW_SHA256_AddBlock(&SymmContext, DeriveBuf, i, true);
    ...
    /* Hash inner digest */
    (void)SBHYBRID_SW_SHA256_AddBlock(&SymmContext, DigestBuf, 32, true);

    /* Get final digest, which is also the derived key */
    SBHYBRID_SW_SHA256_GetDigest(&SymmContext, DigestBuf);
