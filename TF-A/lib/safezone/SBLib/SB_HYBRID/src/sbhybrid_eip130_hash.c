...
#define SBHYBRID_HASH_POLICY     (SBHYBRID_ACCESS_POLICY          | \
                                  ...
                                  EIP130_ASSET_POLICY_SCDIRENCGEN | \
                                  EIP130_ASSET_POLICY_SCUIHASH    | \
                                  EIP130_ASSET_POLICY_SCAHSHA224)
#else
#define SBHYBRID_HASH_POLICY     (SBHYBRID_ACCESS_POLICY       | \
                                  EIP130_ASSET_POLICY_TEMP_MAC | \
                                  EIP130_ASSET_POLICY_SHA224)
#endif
#endif
                                 ...
                                 const uint8_t Finalize)
{
    ...
    Eip130Token_Command_Hash_SetStateASID(&t_cmd, AssetId);
                                    ...
                                    SBLIB_CFG_XM_MAILBOXNR, NULL))
    {
        ...
        if (fFinal)
        {
            ...
            else
            {
                ...
                Context_p->HashDataLength = (uint16_t)DataByteCount;
...
SBHYBRID_EIP130_Hash_RunFsm(SBHYBRID_SymmContext_t * const Context_p)
{
    if (Context_p->HashDigestValid)
    {
                                       ...
                                       SBLIB_CFG_XM_MAILBOXNR))
        {
            ...
            if (sbres == SB_SUCCESS)
            {
                Eip130Token_Result_Hash_CopyState(&t_res,
                                                  SBHYBRID_HASH_ASSETSIZE,
                                                  Context_p->Hash);
            }
