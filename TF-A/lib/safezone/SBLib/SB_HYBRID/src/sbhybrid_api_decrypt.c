...
#if defined(SBIF_RSA) && defined(SB_SW)
    Context_p->AlgoContext_p->algo_type = (uint8_t)(Load_BE32(&Header_p->PubKeyType) >> 29);
#endif
    res = SBHYBRID_Verify_RunFsm(Context_p->AlgoContext_p);
    if (res == SB_SUCCESS)
    {
#if defined(SBLIB_CF_REMOVE_CERTIFICATE_SUPPORT) || defined(SBHYBRID_IMMEDIATE_CERTIFICATE_CHECK)
        // This is success from the Verify proccess
        return res;
        ...
        const SBIF_Certificate_t * Certificates_p;
        const SBIF_Certificate_t * Certificate_p;
        const SBIF_Certificate_t * PrevCertificate_p;
        uint32_t CertNr;

        Certificates_p    = (const SBIF_Certificate_t *)(Header_p + 1);
        ...
        else
        {
            ...
            SBHYBRID_Verify_SetDigest(Context_p->AlgoContext_p,
                                      Context_p->SymmContext.CertDigest[CertNr]);
        }
        // Operation pending, with the next verify started
...
/*----------------------------------------------------------------------------
 * SB_Image_CopyOrDecrypt_Verify
 */
static SB_Result_t
SB_Image_CopyOrDecrypt_Verify(SB_StorageArea_t * const Storage_p,
                              void * PollParam_p,
                              const SBIF_PublicKey_t * PublicKey_p,
                              ...
                              bool DoDecrypt)
{
    ...
    const uint32_t PubKeyType = Load_BE32(&Header_p->PubKeyType) & 0xffffff;
    SB_Result_t res = SB_SUCCESS;

    L_TRACE(LF_SBHYBRID,
            "SB_Image_Verify, Storage_p = %p, PollParam_p = %p, "
    ...
    {
        if ((Hash_p != NULL) || (PublicKey_p == NULL))
        {
            L_DEBUG(LF_SBHYBRID, "ERROR: Invalid hash/public key.");
            return SB_ERROR_VERIFICATION;
        }
    }
    else
    {
        ...
        {
            PublicKey_p = &Header_p->PublicKey;
        }
    }

    // All sanity checks done

#if !defined(SBHYBRID_WITH_EIP130) && !defined(SBHYBRID_WITH_EIP93)
    ...
    if (res == SB_SUCCESS)
    {
        ...
        if (CertificateCount != 0)
        {
            ...
            const SBIF_PublicKey_t * ChkPublicKey_p = PublicKey_p;
            ...
            Certificate_p = (const SBIF_Certificate_t *)(Header_p + 1);
            ...
            for (CertNr = 0; CertNr < CertificateCount; CertNr++, Certificate_p++)
            {
                ...
                if (res == SB_SUCCESS)
                {
                    ...
                    ChkPublicKey_p = (const SBIF_PublicKey_t *)Certificate_p;
                }
                else
                {
            ...
            if (res == SB_SUCCESS)
            {
                // Initialize the Verify with the public key of the last
                // certificate and the image signature
                L_TRACE(LF_SBHYBRID,
                        "Initializing verify for image signature.");
                L_TRACE_PUBKEY(ChkPublicKey_p);
                L_TRACE_SIGNATURE(&Header_p->Signature);
...
#endif /* !SBLIB_CF_REMOVE_CERTIFICATE_SUPPORT */
        {
            // Initialize the Verify with public key and the image
    ...
    // Keep track of Verifies for certificates under processing
    Context_p->CertNr = 0;
    ...
    if ((res == SB_SUCCESS) && (Hash_p != NULL))
    {
        if (PubKeyType != SBIF_PUBKEY_TYPE_IMAGE)
        {
            L_DEBUG(LF_SBHYBRID, "ERROR: Public key is not provided\n");
            res = SB_ERROR_VERIFICATION;
        }
        else
        {
    ...
    if (res == SB_SUCCESS)
    {
        ...
        for (idx = 0; idx < VectorCount; idx++)
        {
            ...
            while (res == SB_SUCCESS && VectorIn.DataLen)
            {
                    ...
                    (Blocklen_in > (SBHYBRID_MAX_SIZE_LAST_DATA_BLOCK + SBHYBRID_HASH_BLOCK)))
                {
                    ...
                    Blocklen_in &= (uint32_t)~((SBHYBRID_HASH_BLOCK) - 1); // align at HASH block size
                }
                ...
                // Continue Verify processing in parallel with symmetric
                // crypto processing
                res = SBHYBRID_Do_Asym_RunFsm(Context_p, PollParam_p, Header_p);
                if (res == SB_SUCCESS)
                {
                    L_DEBUG(LF_SBHYBRID, "ERROR: Unexpected success");
        ...
        if (res == SB_SUCCESS)
        {
            ...
            do
            {
                ...
                // Continue Verify processing in parallel with the
    ...
    if (res == SB_SUCCESS)
    {
        // Wait for the FSM to complete all Verify calculations
        do
        {
            // Provide Verify Processing with the final digest
            // as soon as it is needed
            if (Context_p->NeedFinalDigest)
            {
                SBHYBRID_Verify_SetDigest(Context_p->AlgoContext_p,
                                          Context_p->SymmContext.Digest);
                Context_p->NeedFinalDigest = false;
            }

            // The final call to SBHYBRID_Do_Asym_RunFsm will return with
            // SB_SUCCESS or any of verifications may return with
    ...
    if (res == SB_SUCCESS)
    {
        L_TRACE(LF_SBHYBRID, "SB_Image_Verify: succcessful");
    }
    else
    {
        L_DEBUG(LF_SBHYBRID, "ERROR: SB_Image_Verify failed (%d)", res);
    }
...
SB_Result_t
SB_Image_DecryptWithKeyAndVerify(SB_StorageArea_t * const Storage_p,
                                 void * PollParam_p,
                                 const SBIF_PublicKey_t * PublicKey_p,
                                 const uint8_t * const Hash_p,
                                 const SBIF_Header_t * Header_p,
                                 ...
                                 const uint8_t * ConfidentialityKey)
{
    /* This function processes BLe/BLw images into BLp using SBIF Tools.
       After conversion, standard SB_Image_Verify is used.
       In case of BLp image, optional copying takes place, but nothing else. */

    /* Struct for temporary BLp header to pass to SB_Image_Verify. */
    typedef struct
    {
        SBIF_Header_t Header;
#if SBLIB_CFG_CERTIFICATES_MAX > 0
        SBIF_Certificate_t Certificates[SBLIB_CFG_CERTIFICATES_MAX];
    ...
    const SBIF_Header_t * header_p = Header_p;
    ...
    const uint32_t RsaBytes[3] = {256, 384, 512};
#endif
    SB_Result_t res = SB_SUCCESS;
    ...
    CurveType = (Load_BE32(&Header_p->PubKeyType) >> 24) & 0x1f;
    AlgoType = Load_BE32(&Header_p->PubKeyType) >> 29;
...
#error "SUPPORT_RSA_PKCS or SUPPORT_RSA_PSS must be defined"
#endif
        {
            if (SBIF_RSA_BYTES != RsaBytes[CurveType])
            {
                res = SB_ERROR_VERIFICATION;
            }
        }
    ...
    if (image_type != SBIF_IMAGE_BLTp)
    {
        ...
        else
        {
            ...
            opt.ConvertOptionPointer = (void *)discard_const(ConfidentialityKey);
...
sbiftc_error:
            ...
            case SBIFTC_ERROR_ARGUMENTS:
                L_DEBUG(LF_SBHYBRID,
                        "ERROR: SBIFTOOLS_Image_Convert argument");
                return SB_ERROR_ARGUMENTS;
            case SBIFTC_ERROR_CRYPTO:
                L_DEBUG(LF_SBHYBRID,
                        "ERROR: SBIFTOOLS_Image_Convert crypto");
                return SB_ERROR_HARDWARE;
            case SBIFTC_ERROR_IMAGE_VERSION:
                L_DEBUG(LF_SBHYBRID,
                        "ERROR: SBIFTOOLS_Image_Convert version");
                return SB_ERROR_IMAGE_VERSION;
            case SBIFTC_ERROR_IMAGE_TYPE:
                L_DEBUG(LF_SBHYBRID,
                        "ERROR: SBIFTOOLS_Image_Convert type");
                return SB_ERROR_IMAGE_TYPE;
            default:
                L_DEBUG(LF_SBHYBRID,
    ...
    sbres = SB_Image_CopyOrDecrypt_Verify(Storage_p,
                                          PollParam_p,
                                          PublicKey_p,
                                          Hash_p,
                                          header_p,
...
SB_Result_t
SB_Image_DecryptAndVerify(SB_StorageArea_t * const Storage_p,
                          void * PollParam_p,
                          const SBIF_PublicKey_t * PublicKey_p,
                          const uint8_t * const Hash_p,
                          const SBIF_Header_t * Header_p,
                          ...
                          uint32_t VectorCount)
{
    return SB_Image_DecryptWithKeyAndVerify(Storage_p,
                                            PollParam_p,
                                            PublicKey_p,
                                            Hash_p,
                                            Header_p,
