 ...
 *              Algorithm (ECDSA) and RSA images.
 */
...
#ifndef INCLUDE_GUARD_SB_COMMON_H
#define INCLUDE_GUARD_SB_COMMON_H

#include "public_defs.h"
#include "sbif.h"         // Image format
#include "cfg_sblib.h"
...
typedef struct
{
    /* This structure can get casted to SBHYBRID_Context_t which contains a
     * uint64_t member, therefore ensure uint64_t alignment */
    union
    {
...
/** Verify an ECDSA/RSA format image.
    ...
    @param PublicKey_p
    Must be initialised to ECDSA/RSA public key by the
    caller. The signing tool can generate a char array compatible with
    this structure from a DER format ECDSA/RSA public key.
    ...
    field in the header. SBIF_HEADER_SIZE() macro can be used to
...
/** Verify an ECDSA/RSA format image using specified key.
    ...
    function behaves identically to SB_Image_DecryptAndVerify.
...
*/
SB_Result_t
SB_Image_DecryptWithKeyAndVerify(
    SB_StorageArea_t * const       Storage_p,
    void *                         PollParam_p,
    const SBIF_PublicKey_t *       PublicKey_p,
    const uint8_t * const          Hash_p,
...
/** Get polled during verification.
    ...
    @param PollParam_p
    The value pass to SB_Image_Verify function as PollParam_p
    parameter.
    ...
    SB_Image_Verify function. In this case SB_ECSDA_Image_Verify
...
/** SB_ReadData: Request data to be available.
    ...
    This allows the caller of SB_Image_Verify etc functions to
...
#endif /* INCLUDE_GUARD_SB_COMMON_H */

/* end of file sb_common.h */
