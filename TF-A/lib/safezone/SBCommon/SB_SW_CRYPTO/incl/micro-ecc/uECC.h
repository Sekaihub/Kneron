...
typedef int (*uECC_RNG_Function)(uint8_t *dest, unsigned long int size);
...
typedef struct uECC_HashContext {
    ...
    unsigned int block_size; /* Hash function block size in bytes, eg 64 for SHA-256. */
    unsigned int result_size; /* Hash function result size in bytes, eg 32 for SHA-256. */
