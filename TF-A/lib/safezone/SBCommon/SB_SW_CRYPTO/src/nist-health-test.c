...
drbg_known_answer_test_run(SshDrbgKnownAnswerTestStruct test_case)
{
    ...
    if (c_memcmp(test_case.returned_bits, pseudorandom_buffer, PSEUDORANDOM_BUFFER_LEN))
    {
...
ssh_drbg_known_answer_tests(void)
{
    ...
    number_of_tests = sizeof(drbg_test_cases) / sizeof(SshDrbgKnownAnswerTestStruct);
    ...
    for (i = 0; i < number_of_tests; i++)
    {
        ...
        {
            failed_tests++;
            L_DEBUG(LF_DRBG_FAIL, "Failed known test case %u/%u",
                    i + 1, number_of_tests);
        }
    }
