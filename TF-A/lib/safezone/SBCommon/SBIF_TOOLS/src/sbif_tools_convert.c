                              ...
                              void * const CryptoCtx)
{
    ...
    const uint32_t pubkeyType = BE32_READ(discard_const(&Header_p->PubKeyType));
...
processImageToBLTp:
    ...
    else if (imgType == SBIF_IMAGE_BLTp)
    {
        ...
        if (HeaderNew_p != Header_p)
        {
            ...
            /* Ensure HeaderNew_p matches the original header in size. */
            if (SBIF_GET_HEADER_SIZE(HeaderNew_p, hdrSizeNew) !=
                    hdrSizeNew)
            {
                return SBIFTC_ERROR_ARGUMENTS;
            }
...
SBIFTC_Result_t
SBIFTOOLS_Image_Convert(const SBIF_Header_t * Header_p,
                        uint32_t NewSbifType,
                        uint8_t  NewSbifVersion,
                        SBIF_Header_t * HeaderNew_p,
                        size_t * const HeaderNewSize_p,
                        ...
                        void * const CryptoCtx)
{
    ...
    convertOptionsNew[2].ConvertOptionLength =
        (uint32_t)(sizeof(SBIFTOOLS_ConvertOption_t) * ConvertOptionCount);
