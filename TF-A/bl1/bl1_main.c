...
#if !defined(BL1_ENABLE_MMU) || (BL1_ENABLE_MMU == 1)
...
void bl1_main(void)
{
#if defined(WAGNER_DEF_H) && (TRUSTED_BOARD_BOOT == 1)
	image_desc_t *desc;
	int err;
#endif
	...
	if (image_id == BL2_IMAGE_ID)
		...
		desc = bl1_plat_get_image_desc(NS_BL1U_IMAGE_ID);
		assert(desc != NULL);

		err = bl1_plat_load_auth_image(NS_BL1U_IMAGE_ID, desc);
		if (err != 0) {
			ERROR("Failed to load NS_BL1U.\n");
...
{
	image_desc_t *desc;
	__maybe_unused image_info_t *info;
	int err;
...
#ifdef WAGNER_DEF_H
	err = bl1_plat_load_auth_image(BL2_IMAGE_ID, desc);
#else
