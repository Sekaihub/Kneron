...
struct uvc_still_image_descriptor {
	__u8  bLength;
	__u8  bDescriptorType;
	__u8  bDescriptorSubType;
	__u8  bEndpointAddress;
...
#define UVC_DT_STILL_IMAGE_FRAME_SIZE(p, n)		(6+(p*4)+(n))

#define UVC_STILL_IMAGE_FRAME_DESCRIPTOR(p, n) \
	uvc_still_image_descriptor_##p_##n

#define DECLARE_UVC_STILL_IMAGE_FRAME_DESCRIPTOR(p, n)	\
struct UVC_STILL_IMAGE_DESCRIPTOR(p, n) {		\
	__u8  bLength;					\
	__u8  bDescriptorType;				\
	__u8  bDescriptorSubType;			\
	__u8  bEndpointAddress;				\
...
struct uvc_still_image_trigger_control {
	__u8  bTrigger;
} __attribute__((packed));
