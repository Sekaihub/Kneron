...
static int dw_i2c_plat_probe(struct platform_device *pdev)
{
	...
	ret = device_property_read_u32(&pdev->dev, "snps,bus-capacitance-pf",
				       &dev->bus_capacitance_pF);
	if (ret)
		dev->bus_capacitance_pF = 100;

	dev->clk_freq_optimized = device_property_read_bool(&pdev->dev, "snps,clk-freq-optimized");
