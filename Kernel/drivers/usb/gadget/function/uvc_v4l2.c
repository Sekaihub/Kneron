...
static struct uvc_format_desc *to_uvc_format(struct uvcg_format *uformat)
{
	...
	if (uformat->type == UVCG_FRAMEBASED) {
		uf = to_uvcg_framebased(&uformat->group.cg_item);
		if (!uf)
			return ERR_PTR(-EINVAL);

		memcpy(guid, uf->desc.guidFormat, sizeof(guid));
	}
...
uvc_v4l2_try_format(struct file *file, void *fh, struct v4l2_format *fmt)
{
	...
	calculated_size = uvc_get_frame_size(uformat, uframe);

	/*
	 * If userspace did not set 'sizeimage', or if the provided value
	 * is smaller than the actual frame size, fall back to the driver-
	 * calculated value to prevent buffer underruns or corruption.
	 */
	if (fmt->fmt.pix.sizeimage == 0 ||
	    fmt->fmt.pix.sizeimage < calculated_size) {
		fmt->fmt.pix.sizeimage = calculated_size;
	}
...
uvc_v4l2_qbuf(struct file *file, void *fh, struct v4l2_buffer *b)
{
	...
	if (uvc->still_image_triggered) {
		uvcg_queue_cancel(&video->queue, 0);
		if (b->flags & V4L2_BUF_FLAG_STILL_IMAGE) {
			uvc->still_image_triggered = 0;
			uvcg_dbg(&uvc->func,
				 "Still image buffer queued, trigger cleared\n");
		}
	}
