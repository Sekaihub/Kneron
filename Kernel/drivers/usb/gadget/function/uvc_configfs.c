...
static int uvcg_format_allow_link(struct config_item *src, struct config_item *tgt)
{
	...
	struct uvcg_still_image *still_image_desc;
	struct config_item *streaming, *color_matching, *still_image;
	struct config_group *tgt_parent;
	...
	tgt_parent = to_config_group(tgt->ci_parent);

	if (!strcmp(tgt_parent->cg_item.ci_name, "color_matching")) {
	...
	} else if (!strcmp(tgt_parent->cg_item.ci_name, "still_image")) {
		still_image = config_group_find_item(to_config_group(streaming), "still_image");
		if (!still_image || still_image != tgt->ci_parent) {
			ret = -EINVAL;
			config_item_put(still_image);
			goto end;
		}
		...
		still_image_desc = to_uvcg_still_image(tgt);
		fmt->still_image = still_image_desc;
		still_image_desc->refcnt++;
	} else
		ret = -EINVAL;
...
#define UVCG_STREAMING_HEADER_ATTR(cname, aname, bits, ro...)		\
...
{									\
	...
	return result;							\
									...
									\
UVC_ATTR##ro(uvcg_streaming_header_, cname, aname)

UVCG_STREAMING_HEADER_ATTR(bm_info, bmInfo, 8, _RO);
UVCG_STREAMING_HEADER_ATTR(b_terminal_link, bTerminalLink, 8, _RO);
UVCG_STREAMING_HEADER_ATTR(b_trigger_support, bTriggerSupport, 8, _RO);
UVCG_STREAMING_HEADER_ATTR(b_trigger_usage, bTriggerUsage, 8, _RO);
	...
	struct config_item *item, const char *page, size_t len)
{
	...
	sh->desc.bStillCaptureMethod = num;
	ret = len;
						   ...
						   const char *name)
{
	...
	/* struct uvcg_format *fmt = to_uvcg_format(&group->cg_item); */
	struct uvcg_color_matching *color_match;
	struct config_item *streaming;
...
static struct config_group *uvcg_framebased_make(struct config_group *group,
						   const char *name)
{
	static char guid[] = { /*Declear frame based as H264*/
...
/* -----------------------------------------------------------------------------
 * streaming/still_image/<still_image>/<NAME>
 */

#define UVCG_STILL_IMAGE_PATTERN_ATTR(cname, aname, bits)		\
static ssize_t uvcg_still_image_pattern_##cname##_show(			\
	struct config_item *item, char *page)				\
{									\
	struct uvcg_still_image_pattern *pattern = to_uvcg_still_image_pattern(item); \
	struct f_uvc_opts *opts;					\
	struct config_item *opts_item;					\
	struct mutex *su_mutex = &pattern->item.ci_group->cg_subsys->su_mutex;\
	int result;							\
									\
	mutex_lock(su_mutex); /* for navigating configfs hierarchy */	\
									\
	opts_item = pattern->item.ci_parent->ci_parent->ci_parent->ci_parent; \
	opts = to_f_uvc_opts(opts_item);				\
									\
	mutex_lock(&opts->lock);					\
	result = sprintf(page, "%u\n", le##bits##_to_cpu(pattern->cname)); \
	mutex_unlock(&opts->lock);					\
									\
	mutex_unlock(su_mutex);						\
	return result;							\
									...
									\
static ssize_t uvcg_still_image_pattern_##cname##_store(			\
	struct config_item *item, const char *page, size_t len)		\
{									\
	struct uvcg_still_image_pattern *pattern = to_uvcg_still_image_pattern(item); \
	struct f_uvc_opts *opts;					\
	...
	mutex_lock(&opts->lock);					\
									\
	pattern->cname = cpu_to_le##bits(num);				\
	ret = len;							\
									\
	mutex_unlock(&opts->lock);					\
									\
...
UVC_ATTR(uvcg_still_image_pattern_, cname, aname)

UVCG_STILL_IMAGE_PATTERN_ATTR(w_width, wWidth, 16);
UVCG_STILL_IMAGE_PATTERN_ATTR(w_height, wHeight, 16);

#undef UVCG_STILL_IMAGE_PATTERN_ATTR

#define UVCG_STILL_IMAGE_PATTERN_ATTR_RO(cname, aname, bits)			\
static ssize_t uvcg_still_image_pattern_##cname##_show(			\
	struct config_item *item, char *page)				\
{									\
	struct uvcg_still_image_pattern *pattern = to_uvcg_still_image_pattern(item); \
...
UVC_ATTR_RO(uvcg_still_image_pattern_, cname, aname)

UVCG_STILL_IMAGE_PATTERN_ATTR_RO(b_pattern_index, bPatternIndex, 8);

#undef UVCG_STILL_IMAGE_PATTERN_ATTR_RO

static struct configfs_attribute *uvcg_still_image_pattern_attrs[] = {
	&uvcg_still_image_pattern_attr_b_pattern_index,
	&uvcg_still_image_pattern_attr_w_width,
	&uvcg_still_image_pattern_attr_w_height,
	NULL,
};

static void uvcg_still_image_pattern_release(struct config_item *item)
{
	struct uvcg_still_image_pattern *pattern = to_uvcg_still_image_pattern(item);
...
static struct configfs_item_operations uvcg_still_image_pattern_item_ops = {
	.release	= uvcg_still_image_pattern_release,
};

static const struct config_item_type uvcg_still_image_pattern_type = {
	.ct_item_ops	= &uvcg_still_image_pattern_item_ops,
	.ct_attrs	= uvcg_still_image_pattern_attrs,
	.ct_owner	= THIS_MODULE,
};

static struct config_item *uvcg_still_image_pattern_make(struct config_group *group,
						      const char *name)
{
	struct uvcg_still_image *still = to_uvcg_still_image(&group->cg_item);
	struct uvcg_still_image_pattern *pattern;
	...
	pattern->w_width = 640;
	pattern->w_height = 360;

	still->desc.bNumImageSizePatterns++;
	pattern->b_pattern_index = still->desc.bNumImageSizePatterns;

	list_add_tail(&pattern->entry, &still->patterns);

	config_item_init_type_name(&pattern->item, name,
				   &uvcg_still_image_pattern_type);

	still->desc.bLength = UVC_DT_STILL_IMAGE_FRAME_SIZE(
		still->desc.bNumImageSizePatterns,
		still->desc.bNumCompressionPattern);

	return &pattern->item;
}

static void uvcg_still_image_pattern_drop(struct config_group *group,
				       struct config_item *item)
{
	struct uvcg_still_image_pattern *pattern =
		to_uvcg_still_image_pattern(item);
	struct uvcg_still_image *still = to_uvcg_still_image(&group->cg_item);
...
static struct configfs_group_operations uvcg_still_image_pattern_group_ops = {
	.make_item	= uvcg_still_image_pattern_make,
	.drop_item	= uvcg_still_image_pattern_drop,
};

/* -----------------------------------------------------------------------------
 * streaming/still_image/<NAME>
 */

#define UVCG_STILL_IMAGE_ATTR(cname, aname, ro...)			\
static ssize_t uvcg_still_image_##cname##_show(				\
	struct config_item *item, char *page)				\
{									\
	struct config_group *group = to_config_group(item);		\
	struct uvcg_still_image *still = to_uvcg_still_image(item);     \
	struct f_uvc_opts *opts;					\
	struct config_item *opts_item;					\
	struct mutex *su_mutex = &group->cg_subsys->su_mutex;		\
	int result;							\
...
UVC_ATTR##ro(uvcg_still_image_, cname, aname)

UVCG_STILL_IMAGE_ATTR(b_num_image_size_patterns, bNumImageSizePatterns, _RO);

static ssize_t uvcg_still_image_b_endpoint_address_store(
	struct config_item *item, const char *page, size_t len)
{
	struct config_group *group = to_config_group(item);
	struct uvcg_still_image *still = to_uvcg_still_image(item);
	...
	still->desc.bEndpointAddress = num;
	ret = len;
...
UVCG_STILL_IMAGE_ATTR(b_endpoint_address, bEndpointAddress);

static ssize_t uvcg_still_image_b_num_compression_pattern_store(
	struct config_item *item, const char *page, size_t len)
{
	struct config_group *group = to_config_group(item);
	struct uvcg_still_image *still = to_uvcg_still_image(item);
	...
	if (num == still->desc.bNumCompressionPattern) {
		ret = len;
		goto unlock;
	}

	tmp_buf = krealloc_array(still->desc.bCompression, num,
				 sizeof(u8), GFP_KERNEL | __GFP_ZERO);
	if (!tmp_buf) {
		ret = -ENOMEM;
		goto unlock;
	}

	still->desc.bCompression = tmp_buf;
	still->desc.bNumCompressionPattern = num;
...
UVCG_STILL_IMAGE_ATTR(b_num_compression_pattern, bNumCompressionPattern);

#undef UVCG_STILL_IMAGE_ATTR

static ssize_t uvcg_still_image_b_compression_show(
	struct config_item *item, char *page)
{
	struct config_group *group = to_config_group(item);
	struct uvcg_still_image *still = to_uvcg_still_image(item);
	...
	mutex_lock(&opts->lock);
	for (ret = 0, i = 0; i < still->desc.bNumCompressionPattern; ++i) {
		ret += sprintf(pg, "0x%02x\n", still->desc.bCompression[i]);
		pg = page + ret;
	}
	mutex_unlock(&opts->lock);
...
static ssize_t uvcg_still_image_b_compression_store(struct config_item *item,
						   const char *page, size_t len)
{
	struct uvcg_still_image *still = to_uvcg_still_image(item);
	struct f_uvc_opts *opts;
	...
	iter = b_compression = kcalloc(n, sizeof(u8), GFP_KERNEL);
	if (!b_compression) {
		ret = -ENOMEM;
		goto unlock;
	}

	ret = __uvcg_iter_item_entries(page, len, __uvcg_fill_item_entries,
				       &iter, sizeof(u8));
	if (ret) {
		kfree(b_compression);
		goto unlock;
	...
	kfree(still->desc.bCompression);
	still->desc.bCompression = b_compression;
	still->desc.bNumCompressionPattern = n;
	still->desc.bLength = UVC_DT_STILL_IMAGE_FRAME_SIZE(
		still->desc.bNumImageSizePatterns,
		still->desc.bNumCompressionPattern);
...
UVC_ATTR(uvcg_still_image_, b_compression, bCompression);

static struct configfs_attribute *uvcg_still_image_attrs[] = {
	&uvcg_still_image_attr_b_endpoint_address,
	&uvcg_still_image_attr_b_num_image_size_patterns,
	&uvcg_still_image_attr_b_num_compression_pattern,
	&uvcg_still_image_attr_b_compression,
	NULL,
};

static void uvcg_still_image_release(struct config_item *item)
{
	struct uvcg_still_image *still = to_uvcg_still_image(item);
	struct uvcg_still_image_pattern *pattern, *tmp;

	list_for_each_entry_safe(pattern, tmp, &still->patterns, entry) {
		list_del(&pattern->entry);
		kfree(pattern);
	}
	kfree(still->desc.bCompression);
...
static struct configfs_item_operations uvcg_still_image_item_ops = {
	.release	= uvcg_still_image_release,
};

static const struct config_item_type uvcg_still_image_type = {
	.ct_item_ops	= &uvcg_still_image_item_ops,
	.ct_group_ops	= &uvcg_still_image_pattern_group_ops,
	.ct_attrs	= uvcg_still_image_attrs,
	.ct_owner	= THIS_MODULE,
};

static struct config_group *uvcg_still_image_make(struct config_group *group,
						     const char *name)
{
	struct uvcg_still_image *still;

	still = kzalloc(sizeof(*still), GFP_KERNEL);
	if (!still)
		return ERR_PTR(-ENOMEM);

	INIT_LIST_HEAD(&still->patterns);
	still->desc.bLength = UVC_DT_STILL_IMAGE_FRAME_SIZE(0, 0);
	still->desc.bDescriptorType = USB_DT_CS_INTERFACE;
	still->desc.bDescriptorSubType = UVC_VS_STILL_IMAGE_FRAME;
	still->desc.bEndpointAddress = 0;
	still->desc.bNumImageSizePatterns = 0;
	still->desc.bNumCompressionPattern = 0;
	still->desc.bCompression = NULL;

	config_group_init_type_name(&still->group, name,
				    &uvcg_still_image_type);

	return &still->group;
}

static struct configfs_group_operations uvcg_still_image_grp_group_ops = {
	.make_group	= uvcg_still_image_make,
};

static const struct uvcg_config_group_type uvcg_still_image_grp_type = {
	.type = {
		.ct_item_ops	= &uvcg_config_item_ops,
		.ct_group_ops	= &uvcg_still_image_grp_group_ops,
		.ct_owner	= THIS_MODULE,
	},
	.name = "still_image",
};
	...
	int (*fun)(void *, void *, void *, int, enum uvcg_strm_type type))
{
	...
	list_for_each_entry(f, &h->formats, entry) {
		...
		if (f->fmt->still_image) {
			ret = fun(f->fmt->still_image, priv2, priv3, 0,
				  UVCG_STILL_IMAGE);
			if (ret)
				return ret;
		}
			   ...
			   enum uvcg_strm_type type)
{
	...
	break;
	case UVCG_STILL_IMAGE: {
		struct uvcg_still_image *still = priv1;

		*size += UVC_DT_STILL_IMAGE_FRAME_SIZE(
			still->desc.bNumImageSizePatterns,
			still->desc.bNumCompressionPattern);
	}
	break;
			    ...
			    enum uvcg_strm_type type)
{
	...
	**array = *dest;
	...
	case UVCG_STILL_IMAGE: {
		...
		/* before width/height */
		memcpy(*dest, &still->desc, sizeof(u8) * 5);
		*dest += sizeof(u8) * 5;

		list_for_each_entry(pattern, &still->patterns, entry) {
			*(u16 *)*dest = pattern->w_width;
			*dest += sizeof(u16);
			*(u16 *)*dest = pattern->w_height;
			*dest += sizeof(u16);
		}

		*(u8 *)*dest = still->desc.bNumCompressionPattern;
		*dest += sizeof(u8);
		memcpy(*dest, still->desc.bCompression,
		       still->desc.bNumCompressionPattern);
		*dest += still->desc.bNumCompressionPattern;
	}
	break;
	}
