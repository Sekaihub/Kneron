...
struct uvcg_still_image_descriptor {
	__u8  bLength;
...
struct uvcg_still_image {
	struct config_group group;
	struct uvcg_still_image_descriptor desc;
	struct list_head patterns;
	unsigned int refcnt;
};

static inline struct uvcg_still_image *to_uvcg_still_image(struct config_item *item)
{
	return container_of(to_config_group(item), struct uvcg_still_image,
			    group);
}

struct uvcg_still_image_pattern {
	struct config_item		item;
	u8				b_pattern_index;
	u16				w_width;
	u16				w_height;
...
static inline struct uvcg_still_image_pattern *to_uvcg_still_image_pattern(struct config_item *item)
{
	return container_of(item, struct uvcg_still_image_pattern, item);
}
...
struct uvcg_format {
	...
	struct uvcg_still_image		*still_image;
};
