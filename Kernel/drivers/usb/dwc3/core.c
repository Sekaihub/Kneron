...
static int dwc3_core_num_eps(struct dwc3 *dwc)
{
	struct dwc3_hwparams	*parms = &dwc->hwparams;
	u8			max_num_in_eps;

	dwc->num_eps = DWC3_NUM_EPS(parms);
	max_num_in_eps = DWC3_NUM_IN_EPS(parms);

	dwc->num_in_eps = dwc->num_in_eps ? dwc->num_in_eps : dwc->num_eps / 2;

	if (dwc->num_in_eps > max_num_in_eps) {
		dev_err(dwc->dev, "num_in_eps (%d) exceeds the maximum allowed value (%d)\n",
			dwc->num_in_eps, max_num_in_eps);
		return -EINVAL;
	}
...
static void dwc3_get_properties(struct dwc3 *dwc)
{
	...
	device_property_read_u8(dev, "snps,num-in-eps", &num_in_eps);
	dwc->num_in_eps = num_in_eps;
