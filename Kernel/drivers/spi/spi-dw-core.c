...
static void dw_spi_update_scr0(struct dw_spi *dws, const struct spi_mem_op *op)
{
	u32 trans_t = 0;
	u32 addr_l = 0;
	u32 inst_l = 0;
	...
	scr0 |= FIELD_PREP(QSPI_SCR0_TRANT_MASK, trans_t);
	scr0 |= FIELD_PREP(QSPI_SCR0_ADDRL_MASK, addr_l);
	scr0 |= FIELD_PREP(QSPI_SCR0_INSTL_MASK, inst_l);
	scr0 |= FIELD_PREP(QSPI_SCR0_WAITC_MASK, wait_c);

	dw_writel(dws, DW_SPI_SPI_CTRLR0, scr0);
}

static int dw_spi_fill_cp(struct dw_spi *dws, const struct spi_mem_op *op)
{
	/* send command */
	dw_writel(dws, DW_SPI_DR, (u32)op->cmd.opcode);

	/* send address byte */
	dw_writel(dws, DW_SPI_DR, (u32)op->addr.val);
			  ...
			  struct dw_spi_cfg *cfg)
{
	...
	if (dw_spi_ip_is(dws, PSSI)) {
		...
		cr0 |= FIELD_PREP(DW_PSSI_CTRLR0_SPIFRF_MASK, cfg->frf);
	} else
	...
	if (cfg->tmode == DW_SPI_CTRLR0_TMOD_EPROMREAD ||
		...
		dw_writel(dws, DW_SPI_CTRLR1, cfg->ndf ? (cfg->ndf / ndf_div) - 1 : 0);
...
static int dw_spi_init_mem_buf_enh(struct dw_spi *dws, const struct spi_mem_op *op)
{
	dws->n_bytes = 4;
...
static int dw_spi_write_then_read(struct dw_spi *dws, struct spi_device *spi)
{
	...
	while (room--) {
		if (dws->n_bytes == 1)
			...
			dw_write_io_reg(dws, DW_SPI_DR, cpu_to_be32(*buf32++));
		len--;
	}
	...
	while (len) {
		...
		for (; entries; --entries, --len) {
			if (dws->n_bytes == 1)
				...
				*buf32++ = be32_to_cpu(dw_read_io_reg(dws, DW_SPI_DR));
		}
	}
	return 0;
}
...
static int dw_spi_exec_mem_op_std(struct spi_mem *mem, const struct spi_mem_op *op)
{
	...
	dev_dbg(&dws->master->dev, "%s: cmd:%x mode:%d.%d.%d.%d addr:%llx len:%x\n",
	...
	/* Check if current transfer is a DMA transaction */
	dws->dma_mapped = 0;
	master = mem->spi->master;
	if (dws->dma_ops->can_dma_mem
		&& dws->dma_ops->can_dma_mem(master, mem->spi, op)
	...
	dw_writel(dws, DW_SPI_RXFTLR, dws->fifo_len - 1);

	dw_spi_umask_intr(dws, DW_SPI_INT_RXFI);

	if (dws->dma_mapped) {
		ret = dws->dma_ops->dma_mem_setup(dws, op);
		if (ret)
			return ret;
	}
	 ...
	 */
	if (dws->dma_mapped) {
		ret = dws->dma_ops->dma_mem_exec(dws, mem->spi, op);
	} else {
...
static int dw_spi_exec_mem_op_enh(struct spi_mem *mem, const struct spi_mem_op *op)
{
	struct dw_spi *dws = spi_controller_get_devdata(mem->spi->controller);
	struct dw_spi_cfg cfg;
	struct spi_controller	*master;
	...
	ret = dw_spi_init_mem_buf_enh(dws, op);
	...
	/*
	 * In DW SPI enhance mode (SPI_FRF != 0x0)
	 * there are only two valid combination.
	 * 10 : read only mode for data-in.
	 * 01 : transmit only mode for data-out.
	 */
	cfg.dfs = 32;
	cfg.frf = (op->data.buswidth == 4) ? 2 : 1;
	cfg.freq = clamp(mem->spi->max_speed_hz, 0U, dws->max_mem_freq);
	if (op->data.dir == SPI_MEM_DATA_IN) {
		cfg.tmode = DW_SPI_CTRLR0_TMOD_RO;
		cfg.ndf = op->data.nbytes;
...
static int dw_spi_exec_mem_op(struct spi_mem *mem, const struct spi_mem_op *op)
{
	...
	if (op->data.buswidth > 1)
		ret = dw_spi_exec_mem_op_enh(mem, op);
	else
		ret = dw_spi_exec_mem_op_std(mem, op);
