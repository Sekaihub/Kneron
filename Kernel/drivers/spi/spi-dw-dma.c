			   ...
			   struct spi_device *spi, struct spi_transfer *xfer)
{
	...
	/* check if xfer len is longer then fifo len */
	if (xfer->len < dws->fifo_len)
		return 0;

	/* check if rx buf is 4 byte alignment */
	if ((u64)xfer->rx_buf & 0x3)
		return 0;

	/* check if tx buf is 4 byte alignment */
	if ((u64)xfer->tx_buf & 0x3)
		return 0;
...
static bool dw_spi_can_dma_mem_exec(struct spi_controller *master,
				    ...
				    const struct spi_mem_op *op)
{
	...
	/* check if xfer len is longer then fifo len */
	if (op->data.nbytes < dws->fifo_len)
		return 0;
...
static irqreturn_t dw_spi_dma_mem_exec_handler(struct dw_spi *dws)
{
	dw_spi_check_status(dws, false);
...
static int dw_spi_dma_mem_exec_setup(struct dw_spi *dws,
				     const struct spi_mem_op *op)
{
	u16 imr, dma_ctrl;
	int ret;
	...
	dws->transfer_handler = dw_spi_dma_mem_exec_handler;

	return 0;
}

static int dw_spi_dma_mem_exec_write_then_read(struct dw_spi *dws,
					       struct spi_device *spi,
					       const struct spi_mem_op *op)
{
	struct scatterlist tx_tmp, rx_tmp;
	unsigned long long ms = 500;
	dma_addr_t rx_paddr, tx_paddr;
	int ret;

	dws->master->dev.coherent_dma_mask = DMA_BIT_MASK(32);
	dws->master->dev.dma_mask = &(dws->master->dev.coherent_dma_mask);
	...
	if (dws->rx) {
		/* Prepare DMA Tx sg */
		sg_init_table(&rx_tmp, 1);
		sg_dma_len(&rx_tmp) = dws->rx_len;
		rx_paddr = dma_map_single(&dws->master->dev,
					  dws->rx, dws->rx_len,
					  DMA_FROM_DEVICE);
		sg_dma_address(&rx_tmp) = rx_paddr;
	...
	if (dws->tx) {
		/* Prepare DMA Tx sg */
		sg_init_table(&tx_tmp, 1);
		sg_dma_len(&tx_tmp) = dws->tx_len;
		tx_paddr = dma_map_single(&dws->master->dev,
					  dws->tx, dws->tx_len,
					  DMA_TO_DEVICE);
		sg_dma_address(&tx_tmp) = tx_paddr;
	...
	if (ms == 0) {
		dev_err(&dws->master->dev,
			"spi_mem execute dma transfer timed out\n");
	}

	if (dws->tx) {
		dma_unmap_single(&dws->master->dev,
				 tx_paddr, dws->tx_len,
				 DMA_TO_DEVICE);
	}
...
static int dw_spi_dma_mem_exec(struct dw_spi *dws,
			       struct spi_device *spi,
			       const struct spi_mem_op *op)
{
	int ret;

	ret = dw_spi_dma_mem_exec_write_then_read(dws, spi, op);
	if (ret)
		return ret;

	/* polling TFE */
	while (dw_spi_dma_tx_busy(dws))
		ndelay(100);

	/* polling RFE */
	while (dw_spi_dma_rx_busy(dws))
		ndelay(100);
...
static const struct dw_spi_dma_ops dw_spi_dma_generic_ops = {
	...
	.dma_transfer	= dw_spi_dma_transfer,
	.dma_stop	= dw_spi_dma_stop,
	.can_dma_mem	= dw_spi_can_dma_mem_exec,
	.dma_mem_setup	= dw_spi_dma_mem_exec_setup,
	.dma_mem_exec	= dw_spi_dma_mem_exec,
};
