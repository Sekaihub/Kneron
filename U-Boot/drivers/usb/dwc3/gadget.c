...
static int dwc3_gadget_pullup(struct usb_gadget *g, int is_on)
{
	...
	if (is_on) {
		/*
		 * We are telling dwc3 that we want to use DCFG.NUMP as ACK
		 * TP's NUMP field instead of letting dwc3 itself calculate
		 * that automatically.
		 *
		 * This way, we maximize the chances that we'll be able to get
		 * several bursts of data without going through any sort of
		 * endpoint throttling.
		 */
