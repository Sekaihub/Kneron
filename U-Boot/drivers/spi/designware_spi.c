...
/* Bit fields in SPI_CR0 */
#define QSPI_SCR0_TRANT_MASK		GENMASK(1, 0)
#define QSPI_SCR0_ADDRL_MASK		GENMASK(5, 2)
#define QSPI_SCR0_INSTL_MASK		GENMASK(9, 8)
#define QSPI_SCR0_WAITC_MASK		GENMASK(15, 11)
...
struct dw_spi_priv {
	...
	struct pdma_chan *rxdma;
	struct pdma_chan *txdma;
...
static u32 dw_spi_update_scr0(struct spi_slave *slave, const struct spi_mem_op *op)
{
	u32 trans_t = 0;
	u32 addr_l = 0;
	u32 inst_l = 0;
	...
	return FIELD_PREP(QSPI_SCR0_TRANT_MASK, trans_t)
	     | FIELD_PREP(QSPI_SCR0_ADDRL_MASK, addr_l)
	     | FIELD_PREP(QSPI_SCR0_INSTL_MASK, inst_l)
	     | FIELD_PREP(QSPI_SCR0_WAITC_MASK, wait_c);
}
...
static int dw_spi_of_to_plat(struct udevice *bus)
{
	...
	/* Use 0 as a rx sample delay default */
	plat->rx_sample_dly_ns = dev_read_u32_default(bus, "rx-sample-delay-ns",
						      0);

	if (dev_read_bool(bus, "spi-slave"))
		return -EINVAL;

	dev_info(bus, "max-frequency=%d rx-sample-delay-ns=%d\n",
		 plat->frequency, plat->rx_sample_dly_ns);
...
static void spi_hw_init(struct udevice *bus, struct dw_spi_priv *priv)
{
	...
	dw_write(priv, DW_SPI_TXFTLR, 1);
	dw_write(priv, DW_SPI_RXFTLR, 31);
}

/* get dma info from ofnode */
static void spi_dma_init(struct udevice *bus, struct dw_spi_priv *priv)
{
#if CONFIG_IS_ENABLED(VATICS_PDMA)
	struct pdma_init init;

	dw_write(priv, DW_SPI_DMACR, 0x0);
	dw_write(priv, DW_SPI_DMARDLR, DMA_MAXBURST - 1);
	dw_write(priv, DW_SPI_DMATDLR, DMA_MAXBURST);

	init.paddr = (uint32_t)(uint64_t)priv->regs + DW_SPI_DR;
	init.pdma_descs_count = 64;

	priv->rxdma = pdma_device_request_channel_compat(bus, "rx");

	if (priv->rxdma) {
		dev_info(bus, "rxdma request successfully\n");
		priv->rxdma->init(priv->rxdma, &init);
	}

	priv->txdma = pdma_device_request_channel_compat(bus, "tx");

	if (priv->txdma) {
		dev_info(bus, "txdma request successfully\n");
		priv->txdma->init(priv->txdma, &init);
	}
#endif
	return;
}
	...
	while (max--) {
		...
		if (priv->tx_end - priv->len) {
			if (priv->bits_per_word == 8)
				...
				txw = cpu_to_be16(*(u16 *)(priv->tx));
			else if (priv->bits_per_word == 32)
				txw = cpu_to_be32(*(u32 *)(priv->tx));
		}
	...
	while (max--) {
		...
		if (priv->rx_end - priv->len) {
			if (priv->bits_per_word == 8)
				...
				*(u16 *)(priv->rx) = be16_to_cpu(rxw);
			else if (priv->bits_per_word == 32)
				*(u32 *)(priv->rx) = be32_to_cpu(rxw);
		}
...
static void dma_writer(struct dw_spi_priv *priv)
{
	struct pdma_conf conf = {0};
	struct pdma_prep prep = {0};
	u32 xfer_len = priv->tx_end - priv->tx;

	if (xfer_len == 0)
		return;

	dw_write(priv, DW_SPI_DMACR, 0x2);

	conf.data_unit = (priv->bits_per_word >> 3) - 1;
	conf.intr_en = 1;
	conf.burstlen = DMA_MAXBURST;
	conf.sctrl = 1;
	conf.flowctrl = 0;
	priv->txdma->conf(priv->txdma, &conf);

	prep.xfer_addr = (uint32_t)(uint64_t)priv->tx;
	prep.xfer_len = xfer_len;
	prep.sect_intr = 1;
	prep.sg_intr = 0;
	priv->txdma->prep(priv->txdma, &prep);

	flush_dcache_range((uintptr_t)priv->tx, (uintptr_t)priv->tx_end);

	priv->txdma->xfer(priv->txdma);
...
static void dma_reader(struct dw_spi_priv *priv)
{
	struct pdma_conf conf = {0};
	struct pdma_prep prep = {0};

	u32 xfer_len = priv->rx_end - priv->rx;

	if (xfer_len == 0)
		return;

	dw_write(priv, DW_SPI_DMACR, 0x1);
	...
	conf.sctrl = 1;
	conf.flowctrl = 0;
	priv->rxdma->conf(priv->rxdma, &conf);

	prep.xfer_addr = (uint32_t)(uint64_t)priv->rx;
	prep.xfer_len = xfer_len;
	prep.sect_intr = 1;
	prep.sg_intr = 0;
	priv->rxdma->prep(priv->rxdma, &prep);

	flush_dcache_range((uintptr_t)priv->rx, (uintptr_t)priv->rx_end);

	priv->rxdma->xfer(priv->rxdma);
...
static int dw_spi_exec_op_std(struct spi_slave *slave, const struct spi_mem_op *op)
{
	...
	u8 __aligned(64) op_buf[64];
	u32 cr0, imr;

	log_content("%s: cmd:%x mode:%d.%d.%d.%d addr:%llx len:%x\n",
	...
	priv->rx = NULL;
	priv->rx_end = NULL;
	if (priv->txdma && op_len >= DMA_MAXBURST)
		dma_writer(priv);
	...
	if (read) {
		...
		/*
		 * Enable RX clock gating to
		 * avoid RX fifo overflow.
		 */
		imr = dw_read(priv, DW_SPI_IMR) | IMR_RXFIM;
		dw_write(priv, DW_SPI_IMR, imr);

		dw_write(priv, DW_SPI_SER, 1 << spi_chip_select(slave->dev));
		if (priv->rxdma && op->data.nbytes >= DMA_MAXBURST)
			dma_reader(priv);
		...
		if (readl_poll_timeout(priv->regs + DW_SPI_SR, val,
				       !(val & SR_RF_NOT_EMPT) && !(val & SR_BUSY),
				       RX_TIMEOUT * 1000)) {
			ret = -ETIMEDOUT;
		}
		...
		/*
		 * Enable TX clock gating to
		 * avoid CS de-assertion.
		 */
		imr = dw_read(priv, DW_SPI_IMR) | IMR_TXEIM;
		dw_write(priv, DW_SPI_IMR, imr);
...
static int dw_spi_exec_op_enh(struct spi_slave *slave, const struct spi_mem_op *op)
{
	bool read = op->data.dir == SPI_MEM_DATA_IN;
	int ret = 0;
	struct udevice *bus = slave->dev->parent;
	...
	priv->bits_per_word = 32;
	ndf_div = priv->bits_per_word >> 3;

	cr0 = priv->update_cr0(priv);
	dev_dbg(bus, "cr0=%08x buf=%p len=%u [bytes]\n", cr0, op->data.buf.in,
		op->data.nbytes);

	cr0 |= FIELD_PREP(CTRLR0_SPI_FRF_MASK, ((op->data.buswidth == 4) ? 2 : 1));

	dw_write(priv, DW_SPI_SSIENR, 0);
	dw_write(priv, DW_SPI_CTRLR0, cr0);
	if (read)
		dw_write(priv, DW_SPI_CTRLR1, op->data.nbytes / ndf_div - 1);

	scr0 = dw_spi_update_scr0(slave, op);
	dw_write(priv, DW_SPI_SPI_CTRLR0, scr0);
	dw_write(priv, DW_SPI_SSIENR, 1);

	/* From spi_mem_exec_op, fill opcode and addr*/
	dw_write(priv, DW_SPI_DR, (u32)op->cmd.opcode);
	...
	if (read) {
		...
		priv->rx = op->data.buf.in;
		priv->rx_end = priv->rx + ALIGN(op->data.nbytes, 4);
		...
		priv->tx = op->data.buf.out;
		priv->tx_end = priv->tx + ALIGN(op->data.nbytes, 4);
		...
		dw_write(priv, DW_SPI_SER, 1 << spi_chip_select(slave->dev));
		if (priv->txdma && op->data.nbytes >= DMA_MAXBURST)
			dma_writer(priv);
...
static int dw_spi_exec_op(struct spi_slave *slave, const struct spi_mem_op *op)
{
	...
	if (op->data.buswidth > 1)
		ret = dw_spi_exec_op_enh(slave, op);
	else
		ret = dw_spi_exec_op_std(slave, op);
	return ret;
}
...
static bool dw_spi_supports_op(struct spi_slave *slave, const struct spi_mem_op *op)
{
	int ret;

	if (op->data.buswidth > 4U || op->addr.buswidth > 4U ||
	    op->dummy.buswidth > 1U || op->cmd.buswidth > 1U)
		return false;
...
static int dw_spi_set_speed(struct udevice *bus, uint speed)
{
	...
	plat->rx_sample_dly = DIV_ROUND_CLOSEST(plat->rx_sample_dly_ns,
						NSEC_PER_SEC /
						priv->bus_clk_rate);

	dw_write(priv, DW_SPI_RX_SAMPLE_DLY, plat->rx_sample_dly);

	/* Enable controller after writing control registers */
	dw_write(priv, DW_SPI_SSIENR, 1);

	priv->freq = speed;
	dev_dbg(bus, "speed=%d clk_div=%d rx_dly=%d\n",
		priv->freq, clk_div, plat->rx_sample_dly);
