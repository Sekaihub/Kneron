...
#define SYSC_EQOS_CLK_SET                       0x180
#define SYSC_EQOS_CLK_CLR                       0x184
#define SYSC_EQOS_IRAM_PD_SET                   0x1B0
#define SYSC_EQOS_IRAM_PD_CLR                   0x1B4
...
static int eqos_start_resets_leipzig(struct udevice *dev)
{
	struct eqos_priv *eqos = dev_get_priv(dev);
	int ret;
...
static int eqos_set_tx_clk_speed_leipzig(struct udevice *dev)
{
	struct eqos_priv *eqos = dev_get_priv(dev);
	unsigned int val = 0;

	debug("%s(dev=%p):\n", __func__, dev);
	debug("%s speed = %d\n", __func__, eqos->phy->speed);
	debug("%s phy_mode = %d\n", __func__, eqos->phy->interface);

	switch (eqos->phy->interface) {
	...
	case PHY_INTERFACE_MODE_RGMII_TXID:
		...
		clrsetbits_le32(VPL_SYSC_MMR_BASE + SYSC_PAD_VOLTAGE,
			BIT(21) | BIT(20), BIT(20));
		break;
	...
	case PHY_INTERFACE_MODE_MII:
		...
		break;
	default:
		pr_err("invalid phy mode %d", eqos->phy->interface);
		return -EINVAL;
	}
	writel(val, VPL_SYSC_MMR_BASE + SYSC_EQOS_CTRL);
	return 0;
}
...
static int eqos_adjust_link(struct udevice *dev)
{
	...
	printf("%s speed is %dMbps %s Duplex\n", dev->name,
	       eqos->phy->speed,
				    ...
				    phy_interface_t interface_type)
{
	...
	writel(val, VPL_SYSC_MMR_BASE + SYSC_EQOS_CTRL);
	ioaddr = VPL_EQOSC_MMR_BASE + DW_DMA_BASE_OFFSET;
	writel(readl(ioaddr) | DMAMAC_SRST, ioaddr);

	return 0;
}

__weak int board_eqos_start_clks(struct udevice *dev)
{
#ifdef CONFIG_CLK
	debug("%s(dev=%p):\n", __func__, dev);
	/*
	 * Clock enable register 0
	 * Bit 25: EQOSC Tx clock enable status bit.
	 * Bit 24: EQOSC Rx clock enable status bit.
	 * Bit 23: EQOSC RMII clock enable status bit.
	 * Bit 22: EQOSC PTP REFERENCE clock enable status bit.
	 * Bit 21: EQOSC AXI MMR clock enable status bit.
	 * Bit 20: EQOSC AXI DMA clock enable status bit.
	 */
	writel(0x3F00000, VPL_SYSC_MMR_BASE + SYSC_EQOS_CLK_SET);
	/*
	 * IRAM power down enable register 0
	 * Bit 12: EQOSC internal memory power down status bit.
	 */
	writel(0x1000, VPL_SYSC_MMR_BASE + SYSC_EQOS_IRAM_PD_CLR);
#endif
	debug("%s: OK\n", __func__);
	return 0;
}

__weak int board_eqos_stop_clks(struct udevice *dev)
{
#ifdef CONFIG_CLK
	debug("%s(dev=%p):\n", __func__, dev);
	/*
	 ...
	 * Bit 20: EQOSC AXI DMA clock enable status bit.
	 */
	writel(0x3F00000, VPL_SYSC_MMR_BASE + SYSC_EQOS_CLK_CLR);
	/*
	 * IRAM power down enable register 0
	 * Bit 12: EQOSC internal memory power down status bit.
	 */
	writel(0x1000, VPL_SYSC_MMR_BASE + SYSC_EQOS_IRAM_PD_SET);
#endif
	debug("%s: OK\n", __func__);
	return 0;
}

static int eqos_probe_resources_leipzig(struct udevice *dev)
{
	struct eqos_priv *eqos = dev_get_priv(dev);
	int ret;
	phy_interface_t interface;
...
static phy_interface_t eqos_get_interface_stm32(struct udevice *dev)
{
	...
	/* get phy_mode from uboot env */
	phy_mode = env_get("phy_mode");

	/* if not phy_mode env, get default phy_mode from dts */
	if (!phy_mode)
...
static int eqos_remove_resources_leipzig(struct udevice *dev)
{
#ifdef CONFIG_CLK
	debug("%s(dev=%p):\n", __func__, dev);

	board_eqos_stop_clks(dev);
#endif
	debug("%s: OK\n", __func__);
	return 0;
}
...
static struct eqos_ops eqos_leipzig_ops = {
	.eqos_inval_desc = eqos_inval_desc_generic,
	.eqos_flush_desc = eqos_flush_desc_generic,
	.eqos_inval_buffer = eqos_inval_buffer_generic,
	.eqos_flush_buffer = eqos_flush_buffer_generic,
	.eqos_probe_resources = eqos_probe_resources_leipzig,
	.eqos_remove_resources = eqos_remove_resources_leipzig,
	.eqos_stop_resets = eqos_null_ops,
	.eqos_start_resets = eqos_start_resets_leipzig,
	.eqos_stop_clks = eqos_stop_clks_stm32,
	.eqos_start_clks = eqos_start_clks_stm32,
	.eqos_calibrate_pads = eqos_null_ops,
	.eqos_disable_calibration = eqos_null_ops,
	.eqos_set_tx_clk_speed = eqos_set_tx_clk_speed_leipzig,
	.eqos_get_tick_clk_rate = eqos_get_tick_clk_rate_stm32
};

static const struct eqos_config __maybe_unused eqos_leipzig_config = {
	.reg_access_always_ok = false,
	.mdio_wait = 10000,
	.swr_wait = 50,
	.config_mac = EQOS_MAC_RXQ_CTRL0_RXQ0EN_ENABLED_AV,
...
static const struct udevice_id eqos_ids[] = {
...
#endif
	{
		...
		.data = (ulong)&eqos_leipzig_config
	},
