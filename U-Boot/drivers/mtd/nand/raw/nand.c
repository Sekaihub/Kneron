...
void board_nand_init(void)
{
	...
	for (i = 0; i < CONFIG_SYS_MAX_NAND_DEVICE; i++) {
		...
		ret = uclass_get_device(UCLASS_MTD, i, &dev);

		if (ret && ret != -ENODEV) {
			pr_err("Failed to initialize nand-%d.(error %d)\n",
				i, ret);
			return;

		}
		nand_info[i] = dev_get_uclass_priv(dev);
		if (nand_info[i]) {
			nand_register(i, nand_info[i]);
			pr_info("nand-%d register successfully\n", i);
		}
	}
}
