...
{
#if CONFIG_IS_ENABLED(ARCH_VATICS)
	u8 major, minor;
	unsigned int ver;
...
#if CONFIG_IS_ENABLED(ARCH_VATICS)
	ver = serial_in(((uintptr_t)&com_port->rbr - 4));
	major = ver >> 24;
	minor = (ver >> 16) & 0xff;

	if ((major == 0x2 && minor >= 0x3) || (major > 0x2)) {
		/* Disable Baud Clock */
		if (baud >= 115200)
			divisor = CONFIG_SYS_NS16550_CLK / (16 * baud);
		serial_out(0x00, &com_port->spr);
	}
#endif
	...
	if ((major == 0x2 && minor >= 0x3) || (major > 0x2)) {
		/* Enable Baud Clock */
		divisor = DIV_ROUND_CLOSEST(((CONFIG_SYS_NS16550_CLK -
					   (16 * divisor * baud)) * 64), (16 * baud));

		//printf("2baud_divisor %u\n", divisor);

		/* Delay 1us to sysc clock domain */
		udelay(1);
		if (baud >= 115200)
			serial_out(0x40 | divisor, &com_port->spr);  //DLFRAC
		else
			serial_out(0x40, &com_port->spr);
	}
#endif
